...
import 'package:wink_merchant/models/product.dart'; // UTILISER PRODUCT
...
class StockProductCard extends StatelessWidget {
  ...
  final List<Product> variants; // CORRECTION : On utilise Product
  ...
  Widget build(BuildContext context) {
    ...
    for (var p in variants) {
      ...
      // On utilise le seuil du produit ou 5 par défaut
      int threshold = p.alertThreshold > 0 ? p.alertThreshold : 5;
      totalAlertThreshold += threshold;
      
      if (p.quantity <= threshold) hasCriticalVariant = true;
      
      // CORRECTION : p.sellingPrice au lieu de p.price
    ...
    return Container(
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(16),
          onTap: () {
            // CORRECTION : Navigation avec les bons paramètres attendus par ProductDetailsScreen
            Navigator.push(
              context, 
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text("Rupture ou stock faible", style: TextStyle(fontSize: 11, color: WinkTheme.error, fontWeight: FontWeight.w600)),
                      ],
                    ),
                  )
              ],
